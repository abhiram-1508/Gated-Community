...
function GateVisitors() {
  ...
  return <>
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-[.7fr_1.3fr]">
      <section className="card p-6"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-brand-50 text-brand-600"><ShieldCheck /></div><h2 className="mt-4 section-title">Verify visitor</h2><p className="mt-1 text-sm text-slate-500">Scan the QR using the gate device or enter the six-digit pass.</p><button className="mt-5 flex h-36 w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed border-brand-200 bg-brand-50/40 text-brand-600"><QrCode size={38} /><span className="mt-2 font-bold">Open QR scanner</span></button><div className="my-5 flex items-center gap-3 text-xs text-slate-400"><span className="h-px flex-1 bg-slate-200" /><span>OR ENTER OTP</span><span className="h-px flex-1 bg-slate-200" /></div><div className="flex gap-2"><input maxLength="6" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} className="field text-center font-display text-xl font-bold tracking-[.2em]" placeholder="000000" /><button onClick={verify} className="btn-primary !px-4">Verify</button></div>
