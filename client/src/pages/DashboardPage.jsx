...
import { BadgeCheck, Bell, CalendarCheck, CircleDollarSign, ClipboardCheck, IndianRupee, Plus, ShieldCheck, Siren, TrendingUp, UserCheck, UsersRound, Wrench } from 'lucide-react';
...
function ResidentDashboard() {
  ...
  return <>
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
      ...
      <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-red-600 to-rose-700 p-6 text-white shadow-xl shadow-red-600/15">
        <div className="flex items-start justify-between"><div><div className="text-xs font-bold uppercase tracking-[.16em] text-red-100">Emergency assist</div><h2 className="mt-2 font-display text-2xl font-extrabold">Help is one tap away.</h2></div><Siren size={32} /></div>
        <p className="mt-3 text-sm leading-6 text-red-100">Alert the security desk and community response team with your apartment details.</p><Link to="/app/emergency" className="mt-6 inline-flex rounded-2xl bg-white px-5 py-3 font-bold text-red-600">Activate SOS</Link>
      </section>
    </div>
...
function StaffDashboard() {
  ...
  return <>
    ...
    <section className="card mt-7 p-6"><h2 className="section-title">Priority queue</h2><div className="mt-5 grid gap-4 lg:grid-cols-2">{tasks.map((t) => <div key={t._id} className="rounded-2xl border border-slate-100 p-5 dark:border-slate-800"><div className="flex justify-between"><StatusBadge value={t.priority} /><StatusBadge value={t.status} /></div><h3 className="mt-4 font-display text-lg font-bold capitalize">{t.category} request</h3><p className="mt-1 text-sm text-slate-500">{t.description}</p><div className="mt-4 text-xs text-slate-400">{t.raisedBy?.name || 'Resident'} - {unitLabel(t.unit)}</div><Link to="/app/complaints" className="mt-5 inline-flex text-sm font-bold text-brand-600">Update progress</Link></div>)}</div></section>
  </>;
}
